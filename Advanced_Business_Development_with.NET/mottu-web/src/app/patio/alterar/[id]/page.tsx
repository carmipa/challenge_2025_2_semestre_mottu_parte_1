...
import { Tag, Calendar, Text, Loader2, AlertCircle } from 'lucide-react'; // Removido Info não utilizado
...
export default function AlterarPatioPage() {
    ...
    useEffect(() => {
        ...
        const fetchPatioData = async () => {
            ...
            try {
                ...
                console.error("Erro detalhado no fetch inicial (alterar pátio):", err.response || err);
            } finally {
                setIsFetching(false);
            }
        };
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        setSuccess(null);
        try {
            await PatioService.update(id, formData); // Não espera retorno
            setSuccess(`Pátio (ID: ${id}) atualizado com sucesso!`);
            setTimeout(() => {
                setSuccess(null);
            ...
            console.error("Erro detalhado na atualização (pátio):", err.response || err);
        } finally {
            setIsLoading(false);
        }
    };
