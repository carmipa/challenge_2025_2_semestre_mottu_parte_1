...
import { Tag, Calendar, Text, Loader2, AlertCircle, Stethoscope } from 'lucide-react'; // Usando Stethoscope
import { ZonaRequestDto, ZonaResponseDto } from '@/types/zona'; // Ajuste o path
...
export default function AlterarZonaPage() {
    ...
    useEffect(() => {
        ...
        const fetchZonaData = async () => {
            ...
            try {
                ...
                setFormData({
                    nome: data.nome,
                    dataEntrada: data.dataEntrada, // Já formatado pelo serviço
                    dataSaida: data.dataSaida,     // Já formatado pelo serviço
                    observacao: data.observacao || '',
                });
            } catch (err: any) {
                setError(err.response?.data?.message || err.message || "Falha ao carregar dados da zona.");
                console.error("Erro detalhado no fetch (alterar zona):", err.response || err);
            } finally {
                setIsFetching(false);
            }
        };
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        setSuccess(null);
        try {
            await ZonaService.update(id, formData); // Não espera retorno
            setSuccess(`Zona (ID: ${id}) atualizada com sucesso!`);
            setTimeout(() => {
                setSuccess(null);
            ...
            console.error("Erro detalhado na atualização (zona):", err.response || err);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    if (error && !isFetching && (!formData.nome)) { // Verifica se o nome não foi carregado
        return (
            <>
                <NavBar active="zona-alterar" />
                <main className="container mx-auto p-8 flex justify-center items-center min-h-screen bg-[#012A46]">
            ...
            <main className="container mx-auto px-4 py-12 bg-[#012A46] min-h-screen text-white">
                <div className="bg-slate-900 p-6 md:p-8 rounded-lg shadow-xl w-full max-w-lg mx-auto">
                    <h1 className="flex items-center justify-center gap-2 text-2xl md:text-3xl font-bold mb-8 text-center">
                        <Stethoscope className="text-3xl text-sky-400" /> Alterar Zona (ID: {id}) {/* Ícone Stethoscope */}
                    </h1>
